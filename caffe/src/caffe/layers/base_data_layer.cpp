...
template <typename Dtype>
BasePrefetchingSuperpixelmapDataLayer<Dtype>::BasePrefetchingSuperpixelmapDataLayer(
    const LayerParameter& param)
    : BaseDataLayer<Dtype>(param),
      prefetch_free_(), prefetch_full_() {
  for (int i = 0; i < PREFETCH_COUNT; ++i) {
    prefetch_free_.push(&prefetchsuperpixel_[i]);
  }
}

template <typename Dtype>
void BasePrefetchingSuperpixelmapDataLayer<Dtype>::LayerSetUp(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  BaseDataLayer<Dtype>::LayerSetUp(bottom, top);
  // Before starting the prefetch thread, we make cpu_data and gpu_data
  // calls so that the prefetch thread does not accidentally make simultaneous
  ...
  // seems to cause failures if we do not so.
  for (int i = 0; i < PREFETCH_COUNT; ++i) {
    prefetchsuperpixel_[i].data_.mutable_cpu_data();
    prefetchsuperpixel_[i].labelmap_.mutable_cpu_data();
    prefetchsuperpixel_[i].superpixelmap_.mutable_cpu_data();
    }
#ifndef CPU_ONLY
  if (Caffe::mode() == Caffe::GPU) {
    for (int i = 0; i < PREFETCH_COUNT; ++i) {
      prefetchsuperpixel_[i].data_.mutable_gpu_data();
      prefetchsuperpixel_[i].labelmap_.mutable_gpu_data();
      prefetchsuperpixel_[i].superpixelmap_.mutable_gpu_data();
    }
  }
#endif
  DLOG(INFO) << "Initializing prefetch";
...
template <typename Dtype>
void BasePrefetchingSuperpixelmapDataLayer<Dtype>::InternalThreadEntry() {
#ifndef CPU_ONLY
  cudaStream_t stream;
  if (Caffe::mode() == Caffe::GPU) {
    CUDA_CHECK(cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking));
  ...
  try {
    while (!must_stop()) {
      SuperpixelmapBatch<Dtype>* batch = prefetch_free_.pop();
      load_batch(batch);
#ifndef CPU_ONLY
      if (Caffe::mode() == Caffe::GPU) {
        batch->data_.data().get()->async_gpu_push(stream);
...
template <typename Dtype>
void BasePrefetchingSuperpixelmapDataLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  caffe_copy(batch->superpixelmap_.count(), batch->superpixelmap_.cpu_data(),
...
STUB_GPU_FORWARD(BasePrefetchingSuperpixelmapDataLayer, Forward);
...
INSTANTIATE_CLASS(BasePrefetchingSuperpixelmapDataLayer);
