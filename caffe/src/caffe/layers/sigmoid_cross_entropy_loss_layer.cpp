...
void SigmoidCrossEntropyLossLayer<Dtype>::LayerSetUp(
  ...
  sigmoid_layer_->SetUp(sigmoid_bottom_vec_, sigmoid_top_vec_);
    ...
    weight_ = this->layer_param_.loss_param().weight();
...
void SigmoidCrossEntropyLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < num; ++i) {
     ...
     loss_neg += temp_loss_neg * weight_ * count_pos / (count_pos + count_neg);
  }
    ...
    for (int i = 0; i < num; ++i) {
    	...
    	for (int j = 0; j < dim; j ++) {
        	if (target[i*dim+j] == 1) {
                	...
                	bottom_diff[i * dim + j] *= weight_ * count_pos / (count_pos + count_neg);
        	}
     	}
    }
