...
template <typename Dtype>
class SuperpixelmapBatch {
 public:
  Blob<Dtype> data_, labelmap_, superpixelmap_;
};
template <typename Dtype>
class Batch {
 public:
...
template <typename Dtype>
class BasePrefetchingSuperpixelmapDataLayer :
    public BaseDataLayer<Dtype>, public InternalThread {
 public:
  explicit BasePrefetchingSuperpixelmapDataLayer(const LayerParameter& param);
  // LayerSetUp: implements common data layer setup functionality, and calls
  // DataLayerSetUp to do special data layer setup for individual layer types.
  // This method may not be overridden.
  void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
 ...
 protected:
  virtual void InternalThreadEntry();
  virtual void load_batch(SuperpixelmapBatch<Dtype>* superpixelmapbatch) = 0; 
  BlockingQueue<SuperpixelmapBatch<Dtype>*> prefetch_free_;
  BlockingQueue<SuperpixelmapBatch<Dtype>*> prefetch_full_;
  SuperpixelmapBatch<Dtype> prefetchsuperpixel_[PREFETCH_COUNT];
  Blob<Dtype> transformed_data_;
  Blob<Dtype> transformed_labelmap_;
  Blob<Dtype> transformed_superpixelmap_;
};
